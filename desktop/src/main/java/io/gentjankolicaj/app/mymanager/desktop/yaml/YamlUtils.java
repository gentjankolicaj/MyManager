...
public final class YamlUtils {
    ...
    private static ObjectMapper create() {
        return new ObjectMapper(new YAMLFactory()).registerModules(new JavaTimeModule(), new ParameterNamesModule());
    }

    public static <T> T readFile(String filename, Class<T> clazz) throws IOException {
        return OBJECT_MAPPER.readValue(Thread.currentThread().getContextClassLoader().getResource(filename), clazz);
    }
