...
public final class JsonUtils {
    ...
    private static ObjectMapper createMapper() {
        return new ObjectMapper(new JsonFactory()).registerModules(new ParameterNamesModule(), new JavaTimeModule());
    }

    private static ObjectMapper createObjectMapper() {
        return new ObjectMapper(new JsonFactory()).registerModules(new JavaTimeModule(), new ParameterNamesModule());
    }

    public static <T> T read(String file, Class<T> clazz) throws IOException {
        return JSON_OBJECT_MAPPER.readValue(Thread.currentThread().getContextClassLoader().getResource(file), clazz);
    }
