...
public class CreateEmployee extends JDialog {
  ...
  public CreateEmployee(EmployeeService employeeService,
      EmployeeAdressService employeeAdressService, EmployeeContactService employeeContactService,
      User user) {
  ...
  private int saveDetails() {
    ...
    Employee newEmployee = new Employee(empId, firstName, lastName, middleName, birthday,
        birthplace, Gender.valueOf(sex), jobId, departmentId, projectName, user.getUserId(),
        user.getUserId(), LocalDateTime.now(), LocalDateTime.now());

    try {
      employeeService.saveEmployee(newEmployee);
    } catch (java.sql.SQLIntegrityConstraintViolationException c) {
      WindowUtils.showMessage(this, "New employee must have a job,department and project set.",
          MessageType.ERROR);
      return 0;
    } catch (Exception e) {
