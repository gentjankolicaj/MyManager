...
public class EditEmployee extends JDialog {
	...
	private DateTimeFormatter formatter=DateTimeFormatter.ofPattern("dd MM yyyy");
		...
		private EmployeeAdress oldEmployeeAdress;
		private EmployeeContact oldEmployeeContact;

	/**
	 * Create the dialog.
	 */
	public EditEmployee(EmployeeService employeeService,EmployeeAdressService employeeAdressService,EmployeeContactService employeeContactService,
			User user,Employee oldEmployee,EmployeeAdress oldEmployeeAdress,EmployeeContact oldEmployeeContact) {
	...
	private void populateWorkDetails() {
		...
		if(oldEmployee.getJobId()!=0) {
			Integer jobId=new Integer(jobIdValue);
			jobModel.setSelectedItem(jobId);
		}
		
		if(departmentIdValue!=0) {
			Integer departmentId=new Integer(departmentIdValue);
	...
	private void initComponents() {
		...
		setIconImage(Toolkit.getDefaultToolkit().getImage(
				EditEmployee.class.getResource("/com/mymanager/resources/icons/icons_24x24/icons8-admin-2.png")));
	...
	private int updateDetails() {
		...
		try {
            ...
            c.printStackTrace();
			UtilWindow.showMessage(this, "Employee must have job,department and project set.", MessageType.ERROR);
			return 0;
